...
inline void CreateKeyframeX(Frame& frame)
{
  ...
  transform = Transform::Translate(0.0011, -0.0019, 0.0031) *
...

  {
    ...
    for (int y = 0; y < h; ++y)
    {
      for (int x = 0; x < w; ++x)
      {
        ...
        color[0] += 0.245 * cosf(3.0 * M_PI * xratio);
        color[1] += 0.245 * cosf(3.0 * M_PI * xratio);
        color[2] += 0.245 * cosf(3.0 * M_PI * xratio);
        color[0] += 0.245 * cosf(3.0 * M_PI * yratio);
        color[1] += 0.245 * cosf(3.0 * M_PI * yratio);
        color[2] += 0.245 * cosf(3.0 * M_PI * yratio);
        colors[y * w + x] = color;
      }
    }
...

  {
    ...
    for (int y = 0; y < h; ++y)
    {
      for (int x = 0; x < w; ++x)
      {
        ...
        const float xratio = float(x) / (w - 1);
        const float yratio = float(y) / (h - 1);
        color[0] += 0.245 * cosf(4.0 * M_PI * xratio);
        color[1] += 0.245 * cosf(4.0 * M_PI * xratio);
        color[2] += 0.245 * cosf(4.0 * M_PI * xratio);
        color[0] += 0.245 * cosf(4.0 * M_PI * yratio);
        color[1] += 0.245 * cosf(4.0 * M_PI * yratio);
        color[2] += 0.245 * cosf(4.0 * M_PI * yratio);
        colors[y * w + x] = color;
      }
    }
...
inline double SampleX(int w, int h, const float* values, double u, double v)
{
  ...
  if (keyframe_depth > 0.001)
  {
        ...
        frame_uv[1] >= 1.0 && frame_uv[1] < frame_height - 1.0)
    {
      ...
      if (fabs(frame_depth - Xcp[2]) < 0.099)
      {
        ...
        if (keyframe_normal.SquaredNorm() > 0.001 &&
            frame_normal.Dot(keyframe_normal) > 0.501)
        {
          ...
          const double Ic = SampleX(frame_width, frame_height,
...
void ComputeResidualsX(const Frame& keyframe, const Image& keyframe_intensities_,
...
inline void ComputeJacobianX(const Frame& keyframe, Frame& frame,
    std::vector<Vector6f>& jacobian, std::vector<Vector6f>& visibles)
{
  ...
  for (int i = 0; i < 6; ++i)
  {
    ...
    ComputeResidualsX(keyframe, keyframe_intensities, frame, frame_intensities,
        add_residuals, &add_visibles);

    transform[i] = -step_sizes[i];
    frame.Twc = GetTransformX(transform, base_Twc);

    ComputeResidualsX(keyframe, keyframe_intensities, frame, frame_intensities,
        sub_residuals, &sub_visibles);
...
TEST(ColorTracker, Jacobian)
{
  ...
  ComputeJacobianX(*keyframe, frame, expected, visibles);
