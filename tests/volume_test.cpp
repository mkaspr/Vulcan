...
TEST_F(Volume, UpdateBlockVisibility)
{
  ...
  frame.transform = Transform::Translate(-10, 2, -30);
...
TEST_F(Volume, CreateAllocationRequests)
{
  ...
  frame.transform = Transform::Translate(-10.73, 2.11, -33.54);
  ...
  const Transform inv_transform = frame.transform.Inverse();

  for (int y = 0; y < h; ++y)
  {
    for (int x = 0; x < w; ++x)
    {
      const float d = 1 + 4 * (((x + y) % 100) / 99.0);
      ...
      float t = 0;
      const float tstep = 1E-3;
      Block prev_block;

      while (t < 2 * trunc_length)
      {
        const Vector3f current = origin + t * dir;
        const int bx = current[0] * inv_block_length;
        const int by = current[1] * inv_block_length;
        const int bz = current[2] * inv_block_length;
        const Block curr_block(bx, by, bz);

        if (t > 0 && curr_block == prev_block)
        {
          t += tstep;
          continue;
        }
