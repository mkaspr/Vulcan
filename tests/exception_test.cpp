...
TEST(Exception, AssertMessage)
{
  ...
  try
  {
    ...
    VULCAN_ASSERT_MSG(0 > 1, expected_text);
  }
  ...
  ASSERT_TRUE(thrown);
  ASSERT_NO_THROW(VULCAN_ASSERT_MSG(0 < 1, ""));
}
...
TEST(Exception, Assert)
{
  ...
  ASSERT_TRUE(thrown);
  ASSERT_NO_THROW(VULCAN_ASSERT(0 < 1));
}
...
TEST(Exception, DebugMessage)
{
  ...
  {
    expected_line = __LINE__ + 1;
    VULCAN_DEBUG_MSG(0 > 1, expected_text);
  }
  catch (const Exception& exception)
  {
    const std::string expected_what = expected_file + "(" +
  ...
  ASSERT_NO_THROW(VULCAN_DEBUG_MSG(0 < 1, ""));
}
...
TEST(Exception, Debug)
{
  ...
  ASSERT_NO_THROW(VULCAN_DEBUG(0 < 1));
}
